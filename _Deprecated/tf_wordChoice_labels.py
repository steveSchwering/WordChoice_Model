...
if __name__ == '__main__':
	...
	x = tf.placeholder(tf.int32, [num_batches, num_timeSteps], name = 'message_placeholder')
	y = tf.placeholder(tf.int32, [num_batches, num_timeSteps], name = 'phonology_placeholder') 
	init_state = tf.zeros([num_batches, size_layer_hidden])
	rnn_inputs = tf.one_hot(x, size_layer_message)
	"""
	...
	"""
	# RNN
	cell = tf.contrib.rnn.BasicRNNCell(size_layer_hidden)
	...
	with tf.variable_scope('softmax'):
		W = tf.get_variable('W', [size_layer_hidden, size_layer_phonology])
		b = tf.get_variable('b', [size_layer_phonology], initializer=tf.constant_initializer(0.0))
	logits = tf.reshape(tf.matmul(tf.reshape(rnn_outputs, [-1, size_layer_hidden]), W) + b, [num_batches, num_timeSteps, size_layer_phonology])
